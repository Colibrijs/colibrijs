...
export class PageRemoveTO {
  ...
  constructor(options: Options) {
    ...
    this.testId = options.testId ?? 'page-remove';
  }
  ...
  async confirm(): Promise<void> {
    await this.step('Подтверждаю удаление', async () => {
      ...
      const confirm = await screen.findByTestId('page-remove__confirm');
      await userEvent.click(confirm);
    });
  }

  getErrorMessage(): Promise<HTMLElement> {
    return screen.findByTestId('page-remove__error');
  }

  getSuccessMessage(): Promise<HTMLElement> {
    return screen.findByTestId('page-remove__success');
  }
}
